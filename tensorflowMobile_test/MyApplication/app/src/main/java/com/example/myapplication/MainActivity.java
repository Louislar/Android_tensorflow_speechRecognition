...
public class MainActivity extends AppCompatActivity {
    ...
    private String INPUT_LAYER_NAME= "dense_1_input";
    private String OUTPUT_LAYER_NAME= "output_1";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView textView_predict_class = findViewById(R.id.textView_predict_class);
        TextView textView_predict_confidence = findViewById(R.id.textView_predict_confidence);
        new AsyncTask<Integer,Integer,Integer>() {

            @Override
            protected Integer doInBackground(Integer... params) {
                //想在背景做的事
                tf = new TensorFlowInferenceInterface(getAssets(), "file:///android_asset/minst_trained_model.pb");
                float[] input_float_data = new float[28*28];
                for(int i=0;i<28*28;i++)
                {
                    input_float_data[i]=1;
                }
                float[] predictions = new float[1000];
                tf.feed(INPUT_LAYER_NAME, input_float_data, 1, 784);
                tf.run(new String[]{OUTPUT_LAYER_NAME});
                tf.fetch(OUTPUT_LAYER_NAME, predictions);
                Object[] results = argmax(predictions);
                int class_index = (Integer) results[0];
                float confidence = (Float) results[1];
                try{
                    final String conf = String.valueOf(confidence * 100).substring(0,5);
                    textView_predict_class.setText(class_index + " : " + conf + "%");
                }
                catch (Exception e){
                }
...
//        tf = new TensorFlowInferenceInterface(getAssets(), "file:///android_asset/my_frozen_graph.pb");
//        float[] input_float_data = new float[28*28];
//        for(int i=0;i<28*28;i++)
//        {
//            input_float_data[i]=0;
//        }
//        float[] predictions = new float[1000];
//        tf.feed(INPUT_LAYER_NAME, input_float_data, 1, 28, 28);
//        tf.run(new String[]{OUTPUT_LAYER_NAME});
//        tf.fetch(OUTPUT_LAYER_NAME, predictions);
//        Object[] results = argmax(predictions);
//        int class_index = (Integer) results[0];
