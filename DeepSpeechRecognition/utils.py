...
def data_hparams():
    params = tf.contrib.training.HParams(
        ...
        stcmd=False,
        batch_size=4,
        data_length=None, # 更動就會出現錯誤訊息，還未解決，已解決，訓練時與測試時要一樣
        shuffle=True)
    return params
...
class get_data():
    ...
    def get_am_batch(self):
        ...
        while 1:
            ...
            for i in range(len(self.wav_lst) // self.batch_size):   # //只是取到整數而已
                wav_data_lst = []
                label_data_lst = []
                begin = i * self.batch_size
                end = begin + self.batch_size
                sub_list = shuffle_list[begin:end]  # 取得一個batch大小的data數量(筆數)
