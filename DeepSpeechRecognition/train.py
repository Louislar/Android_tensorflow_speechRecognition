"""
重要參考網頁: http://www.pianshen.com/article/7114277129/
"""
import os
...
data_args.batch_size = 4
data_args.data_length = None
# data_args.data_length = 10  # 改成None就會抱錯
data_args.shuffle = True
...
am.ctc_model.fit_generator(batch, steps_per_epoch=batch_num, epochs=10, callbacks=[checkpoint], workers=1, use_multiprocessing=False, validation_data=dev_batch, validation_steps=200)
am.ctc_model.save_weights('logs_am/model.h5')   # 這種儲存方式，在讀取時會沒辦法直接load
am.ctc_model.save('logs_am/whole_model.h5')     # 所以改用這種儲存方式
